...
function getSalesDb(): Database.Database | null {
  ...
  const dbPath = path.join(DATA_DIR, 'groups', 'telegram_tpg_uncaged', 'messages.db');
...
function cleanSenderName(rawName: string | undefined): string {
  ...
  const cleaned = rawName.replace(/[^\p{L}\s']/gu, ' ').replace(/\s+/g, ' ').trim();
...
function detectMonthlyOrAnnual(content: string, amount: number): 'monthly' | 'annual' {
  const c = content.toLowerCase();
  // Explicit annual premium indicators
  if (/\bap\b|annual premium|per year|\/year|\/yr/i.test(content)) return 'annual';
...
function recordSale(msg: NewMessage, amount: number, amountStr: string): void {
  ...
  try {
    ...
    db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(
      msg.id, msg.sender, cleanName, amount, product,
      msg.timestamp, annualPremium, isMonthly ? 1 : 0, msg.content
    );
    logger.info({ sender: cleanName, amount: amountStr, mode, ap: annualPremium }, 'Sale recorded');
  } catch (err) {
...
function extractProduct(content: string): string {
  ...
  if (c.includes('amam') || c.includes('american amicable') || c.includes('am am')) return 'American Amicable';
...
const CARRIERS = [
  'trans', 'transamerica', 'transexp', 'trans express', 'transsolutions', 'trans solutions',
  'aflac', 'americo', 'moo', 'mutual of omaha', 'mutual',
  'aig', 'cica', 'ethos', 'corebridge', 'aetna',
  'amam', 'am am', 'american amicable',
  'igo', 'asap', 'royal neighbors', 'cvs', 'prosperity',
  'assurity', 'sagicor', 'foresters',
];

const CARRIER_RE = new RegExp(CARRIERS.map(c => c.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')).join('|'), 'i');
...
const CELEBRATION_RE = /🎉|🎊|🔥|💪|⚔️|❤️|🌺|💕|🌈|approved|graded|rewrite|wrote|closed|boom/i;
...
const REJECT_RE = /lead (pack|store)|overflow|tcpa|earn \$|question|anyone know|does anyone|how do|can you|should i|when putting|zoom|training|happening now|scratch th(is|at)|(?<!\$\d{1,6}[\s\S]{0,100})cancel(led)?|fell through|didn'?t go through|won'?t go through|not going through|backed out|changed their mind|never\s?mind|disregard|(?<!\w)void(?!\w)/i;

/** Daily recap patterns — these are summaries, not individual sales. */
const RECAP_RE = /\b(dials?|pick\s?ups?|presentations?|contacts?|closed apps?|talk\s?time|hrs?\b|hours?\b)\b.*\b(dials?|pick\s?ups?|presentations?|contacts?|closed apps?|talk\s?time|hrs?\b|hours?\b)\b/is;
// Single activity word with a total/close reference = recap
const RECAP_WITH_TOTAL_RE = /\b\d+\s*(dials?|presentations?|pick\s?ups?|contacts?)\b[\s\S]*\b(close[ds]?|sales?)\b[\s\S]*\$/i;
const AP_TOTAL_RE = /\bAP\b/i;

/** Celebration templates grouped by STYLE so we never repeat the same vibe back to back. */
const CELEBRATION_STYLES: { style: string; templates: ((name: string, amount: string) => string)[] }[] = [
  {
    style: 'hype',
    templates: [
      (name, amount) => `🔥🔥🔥 ${name} JUST DROPPED ${amount}!!! Who's next?! ⚔️`,
      (name, amount) => `BOOM 💥 ${name} with ${amount} ON THE BOARD! Let's GOOOOO! 🚀`,
      (name, amount) => `LETS GOOOOO 🚀🚀🚀 ${name} just closed ${amount}!! DIAL, CLOSE, REPEAT! ⚔️🔥`,
    ],
  },
  {
    style: 'respect',
    templates: [
      (name, amount) => `${name} just put ${amount} on the board. Quiet work, loud results. 💪`,
      (name, amount) => `${amount} from ${name}. That's what consistency looks like. 🔥`,
      (name, amount) => `Another one from ${name}. ${amount}. The scoreboard tells the story. 🏆`,
    ],
  },
  {
    style: 'competitive',
    templates: [
      (name, amount) => `${name} with ${amount}!! Who's matching this energy today?! 🔥`,
      (name, amount) => `${amount}!!! ${name} is putting the team on notice! Keep that same energy!! ⚔️`,
  ...
  },
  {
    style: 'team',
    templates: [
      (name, amount) => `${name} just added ${amount} to the team total! Every sale matters, every dial counts! 💪🔥`,
      (name, amount) => `${amount} from ${name}! That's what happens when you pick up the phone! 📞🔥`,
      (name, amount) => `${name} with ${amount}! The phones are working today!! Who else is dialing?! ⚔️`,
    ],
  },
  {
    style: 'short',
...
const MULTI_CELEBRATIONS = [
  (name: string, total: string, count: number) => `🔥🔥🔥 ${name} JUST CLOSED ${count} POLICIES!!! ${total} TOTAL!! That's how you protect a family! ⚔️💪`,
  (name: string, total: string, count: number) => `${name} with ${count} sales, ${total} on the board!! When BOTH of them say yes?! THAT'S the goal! 🔥🔥`,
  (name: string, total: string, count: number) => `${name} said WHY STOP AT ONE?! ${count} policies, ${total} total!! Who else is doubling up?! 🔥⚔️`,
  (name: string, total: string, count: number) => `${count} sales. ${total}. ${name} didn't leave the table early. 💪🔥`,
  (name: string, total: string, count: number) => `${name} stacked ${count} for ${total}!! Protected the whole household!! 🏆🔥`,
  (name: string, total: string, count: number) => `${total} from ${name}! ${count} policies in one shot! That's elite. ⚔️`,
];
...
function detectSales(msg: NewMessage): { name: string; amount: string }[] {
  ...
  const hasSaleLanguage = /approved|closed|wrote|got|sale|ASAP|effective|E-?check|DirExpress|existing client|rewrite|first policy|second policy|husband.*wife|mom|dad/i.test(content);
...
export function checkAndCelebrateSale(
  ...
  if (sales.length >= 2) {
    const total = sales.reduce((sum, s) => sum + parseFloat(s.amount.replace(/[$,]/g, '')), 0);
    const totalStr = `$${total.toFixed(2)}`;
    const template = MULTI_CELEBRATIONS[celebrationIndex % MULTI_CELEBRATIONS.length];
    celebrationIndex++;
    text = template(name, totalStr, sales.length);
  } else {
    // Pick a style that's DIFFERENT from the last one used
    const availableStyles = CELEBRATION_STYLES.filter(s => s.style !== lastStyleUsed);
    const chosenStyle = availableStyles[Math.floor(Math.random() * availableStyles.length)] || CELEBRATION_STYLES[0];
    const template = chosenStyle.templates[Math.floor(Math.random() * chosenStyle.templates.length)];
    ...
    if (recentTexts.includes(text)) {
      const fallback = chosenStyle.templates.find(t => !recentTexts.includes(t(name, sales[0].amount)));
  ...
  setTimeout(() => {
    ...
    logger.info({ sender: name, salesCount: sales.length, amounts: sales.map(s => s.amount) }, 'Sales celebration sent');
